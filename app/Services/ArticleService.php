...
class ArticleService
{
    ...
    {
        // 获取每个订阅的数量
        $temp_counts = ArticleSub::select('feed_id', DB::raw('count(*) as total'))->where('user_id', $user->id)
            ->where('status', $status)
            ->groupBy('feed_id')
            ->get();
    ...
    {
        return ArticleSub::whereIn('id', $ids)
            ->where('user_id', Auth::user()->id)
            ->where('status', 'unread')->update([
                'status' => $status,
    ...
    {
        return ArticleSub::where('feed_id', $feedId)
        ->where('user_id', Auth::user()->id)
        ->where('status', 'unread')->update([
            'status' => $status,
            'updated_at' => date('Y-m-d H:i:s')
