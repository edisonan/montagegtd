 ...
 * 文章简报 v2 生成服务（对比版管线）
 *
 * 与 v1 的差异（保持候选口径完全一致，仅修 LLM 侧）：
 * 1. 输出 max_tokens 提到 4096，避免结构化 JSON 在 1024 token 处被截断；
 * 2. 标签聚合移出 LLM 输出，改由 AI 画像标签确定性聚合（top 15），大幅降低输出预算；
 * 3. LLM 失败自动重试一次（附「直接输出 JSON，禁止思考过程」指令），
 *    针对 reasoning 模型把预算消耗在 reasoning_content 导致正文为空的问题；
 * 4. 兜底结果显式标记 fallback=1 并记录真实失败原因（v1 恒为 success、无原因）；
 * 5. topic_count 存「真实热点数」而非候选数，候选数单独存 candidate_count；
 * 6. 趋势/信号支持多佐证：LLM 输出 article_ids（1~5 条），全部相关候选文章逐条展示。
 *
 * @author edison.an
 */
class BriefingV2GenerationService
{
    const MAX_PULL_HOURS = 24;
    // deepseek-v4-flash 为推理模型，会把大量输出预算消耗在 reasoning_content 上；
    // 4096 仍会顶格截断正文，故提高到 8192 给正文留足余量。
    ...
    /**
     * 为指定用户/配置生成 v2 简报。仅取 config->user_id 对应的订阅文章。
     *
     * @param int $configId
    ...
    public function generateForConfig($configId, $userId = null)
    {
        ...
        // 注意：v2 不写 briefing_configs.last_generated_at，避免影响 v1 展示，保持两版隔离

        if (!empty($content['fallback'])) {
            Log::warning('briefing v2 fallback, config=' . $config->id . ', page=' . $page->id
                . ', reason=' . (string)$page->error_message . ', candidates=' . $candidates->count());
        } else {
            Log::info('briefing v2 generated, config=' . $config->id . ', page=' . $page->id
    ...
    /**
     * 调度使用：按定时时间将到期的配置逐一生成 v2 简报任务（预留，默认未启用）
     */
    ...
    /**
     * v2 生成：最多两次尝试（第二次附「禁止思考过程」指令），失败返回兜底并记录原因。
     */
    ...
    protected function buildResultFromDecoded($config, array $decoded, Collection $candidates, $modelName)
    {
        ...
        // v2：标签聚合一律走确定性聚合（AI 画像标签），并按文章数排序取 top N，避免「标签墙」
    ...
    protected function buildFallbackResult($config, Collection $candidates, $modelName)
    {
        ...
        return array(
            'title' => '前' . max(1, (int)$config->pull_hours) . '小时文章简报',
            // v2：topic_count 存真实热点数（5），不再用候选数虚报「主题数量」
            'hot_topics' => $topics,
