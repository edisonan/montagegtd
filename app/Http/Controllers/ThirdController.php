...
class ThirdController extends Controller {
	...
	public function fanfouIndex(Request $request) {
		...
		return redirect ( ( string ) $aurl )->with ( 'message', '操作成功!' );
	}
	...
	public function fanfouCallback(Request $request) {
		...
		$o = new OAuth ( config ( "services.fanfou.client_id" ), config ( "services.fanfou.client_secret" ), $temp ['oauth_token'], $temp ['oauth_token_secret'] );
		
		// 获取access_token
		$last_key = $o->getAccessToken ( $temp ['oauth_token'] );
		
		// 创造一个新的请求
		$ff_user = new FFClient ( config ( "services.fanfou.client_id" ), config ( "services.fanfou.client_secret" ), $last_key ['oauth_token'], $last_key ['oauth_token_secret'] );
		...
		echo '<a href="/">' . $result_arr ['name'] . '! process finish,return index!</a>';
		exit ();
		exit ();
	}
	...
	public function testFave(Request $request) {
		...
		if (empty ( $third )) {
			return redirect ( 'third/fanfouIndex' )->with ( 'message', '操作成功!' );
		}
