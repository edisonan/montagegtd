...
@section('title', '技能计划 - 蒙太奇')
@section('description', '管理您的技能发展计划，制定成长计划，追踪学习进度。')
...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">技能计划管理</h1>
                    <p class="text-gray-600 mt-1">设定并追踪您的技能成长计划</p>
                </div>
        ...
        <div class="card mb-8">
            ...
            <div class="p-6">
                <form action="javascript:void(0)" method="POST" class="space-y-5" id="planCreateForm">
                    <!-- 技能名称输入 -->
                    <div class="space-y-2">
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p id="planCountText" class="text-sm text-gray-500 mt-1">共 0 个技能计划</p>
                    </div>

                    <span id="planCountBadge" class="hidden text-xs px-3 py-1 bg-blue-100 text-blue-600 rounded-full font-medium"></span>
                </div>
            </div>

            <div class="p-6">
                <div id="plansListContainer" class="text-center py-12 text-gray-500">加载计划中...</div>
            </div>
        </div>
    </div>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content max-w-md">
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-700 text-center">确定要删除这个技能计划吗？所有相关记录也将被删除。</p>
            </div>
    ...
    <script>
        ...
        function updateGoalCount() {
            var count = planState.items.length;
            var text = document.getElementById('planCountText');
            var badge = document.getElementById('planCountBadge');
            if (text) text.textContent = '共 ' + count + ' 个技能计划';
            if (badge) {
                if (count > 0) {
                    badge.classList.remove('hidden');
                    badge.textContent = count + ' 个计划';
                } else {
        ...
        function renderGoals() {
            ...
            if (!planState.items.length) {
                container.innerHTML = '<div class="text-center py-12"><div class="w-16 h-16 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center mx-auto mb-4"><i class="fas fa-bullseye text-gray-400 text-xl"></i></div><h3 class="text-lg font-medium text-gray-700 mb-2">暂无技能计划</h3><p class="text-gray-500 max-w-md mx-auto mb-6">您还没有设定任何技能计划，从上面的表单开始添加第一个技能计划吧！</p></div>';
                updateGoalCount();
                return;
            }

            var html = '<div class="space-y-3">';
            planState.items.forEach(function(plan) {
                var progress = Number(plan.progress || 0);
                var progressHtml = progress > 0
                    ...
                    : '';
                html += '<div id="' + Number(plan.id) + '" class="plan-item card hover:border-gray-300 transition-colors"><div class="p-4"><div class="flex items-center justify-between"><div class="flex items-center gap-4 flex-1 min-w-0"><div class="flex-shrink-0 w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-lg flex items-center justify-center"><i class="fas fa-bullseye text-white"></i></div><div class="flex-1 min-w-0"><h3 class="font-medium text-gray-800 text-lg truncate">' + escapeHtml(plan.name) + '</h3><div class="flex items-center gap-3 mt-1"><span class="text-xs text-gray-500"><i class="fas fa-calendar-plus mr-1"></i>' + formatDate(plan.created_at) + '</span>' + progressHtml + '</div></div><div class="flex items-center gap-2 flex-shrink-0 ml-4"><a href="/plan/' + Number(plan.id) + '" class="p-2 text-gray-400 hover:text-blue-500 rounded-lg hover:bg-blue-50 transition-colors" title="编辑技能"><i class="fas fa-edit"></i></a><button type="button" data-plan-id="' + Number(plan.id) + '" class="delete-plan p-2 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50 transition-colors" title="删除技能"><i class="fas fa-trash-alt"></i></button></div></div></div></div></div>';
            });
            html += '</div>';
        ...
        function loadGoals() {
            ...
            apiRequest('GET', '/plans', { status: 1 }).then(function(response) {
                if (!response || response.code !== 9999) throw new Error((response && response.msg) || '加载失败');
                planState.items = Array.isArray(response.result && response.result.plans) ? response.result.plans : [];
                renderGoals();
            }).catch(function() {
                if (container) container.innerHTML = '<div class="text-center py-12 text-gray-500">计划加载失败，请稍后重试</div>';
            });
        }

        document.addEventListener('DOMContentLoaded', function() {
            var deleteModal = document.getElementById('deleteModal');
            var planForm = document.getElementById('planCreateForm');
            ...
            document.addEventListener('click', function(e) {
                var deleteBtn = e.target.closest('.delete-plan');
                if (!deleteBtn) return;
                planState.currentGoalId = deleteBtn.getAttribute('data-plan-id');
                deleteModal.classList.add('show');
            });

            confirmDeleteBtn.addEventListener('click', function() {
                if (!planState.currentGoalId || !apiRequest) return;
                var button = this;
                var original = button.innerHTML;
                button.disabled = true;
                button.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>删除中...';
                apiRequest('DELETE', '/plans/' + planState.currentGoalId, {}).then(function(response) {
                    if (response && response.code === 9999) {
                        planState.items = planState.items.filter(function(item) { return String(item.id) !== String(planState.currentGoalId); });
                        renderGoals();
                        showNotification('success', '技能计划已删除');
                    } else {
            ...
            if (planForm) {
                planForm.addEventListener('submit', function(e) {
                    ...
                    apiRequest('POST', '/plans', { name: nameInput.value.trim() }).then(function(response) {
                        if (response && response.code === 9999) {
                            nameInput.value = '';
                            loadGoals();
                            showNotification('success', '技能计划已添加');
                        } else {
